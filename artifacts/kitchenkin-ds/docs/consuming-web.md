...
```css
@import "@workspace/kitchenkin-ds/styles.css";
```
...
`node_modules/@workspace/kitchenkin-ds/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/kitchenkin-ds/components/ui/button";
import { cn } from "@workspace/kitchenkin-ds/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/kitchenkin-ds/hooks/use-toast";
```
...
`@workspace/kitchenkin-ds/components/ui/button`. Run the app's typecheck
