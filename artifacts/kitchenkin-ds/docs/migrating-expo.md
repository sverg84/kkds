...
`artifacts/kitchenkin-ds/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/kitchenkin-ds/lib/native-theme` and
  `@workspace/kitchenkin-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/kitchenkin-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/kitchenkin-ds` and retains only
