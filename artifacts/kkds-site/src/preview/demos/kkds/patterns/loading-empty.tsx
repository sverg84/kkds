...
type State = "loading" | "empty" | "no-results";
    ...
    <div className="space-y-10">
      ...
      <div className="rounded-xl border bg-card p-5 space-y-3 text-sm">
        ...
        <p className="text-muted-foreground leading-relaxed">
          Skeleton layouts must match live content dimensions exactly — use the
          same count as your page size and the same responsive grid class. Empty
          states are warm and encouraging, not apologetic. Every empty state has
        ...
        <div className="space-y-3">
          <p className="text-xs text-muted-foreground">
            <strong className="text-foreground">
              Intrinsically empty state.
            </strong>{" "}
            The collection exists but has no items. Heading invites action —
            never apologises. CTA leads to the creation flow.
          </p>
      ...
      {activeState === "no-results" && (
        <div className="space-y-3">
          <p className="text-xs text-muted-foreground">
            <strong className="text-foreground">No-results state.</strong> The
            search or filter yielded zero matches. Heading is contextual
            (includes the query). CTA clears filters so the user can start over.
          </p>
          <Empty className="border bg-card">
            <EmptyHeader>
              <EmptyMedia variant="icon">
      ...
      <div className="rounded-xl border bg-card overflow-hidden">
        ...
        <div className="divide-y text-sm">
          {[
            ...
            <div
              key={state}
              className="grid grid-cols-[80px_1fr_1fr_1fr] gap-4 px-5 py-3 text-xs"
            >
